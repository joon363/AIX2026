...
void forward_convolutional_layer_q(network net, layer l, network_state state)
{
    ...
    if (run_single_image_test) {
        ...
        snprintf(file_input_femap, sizeof(file_input_femap), "/app/bin/log_feamap/CONV%02d_input.hex", state.index);
    ...
    if (run_single_image_test) {
        ...
        snprintf(file_output_femap, sizeof(file_output_femap), "/app/bin/log_feamap/CONV%02d_output.hex", state.index);
...
void do_quantization(network net) {
    ...
    if (fp_mul) {
        int idx; float mul;
        while (fscanf(fp_mul, "%d %f", &idx, &mul) == 2) {
            if (idx >= 0 && idx < 200) loaded_multipliers[idx] = mul;
        }
        fclose(fp_mul);
    } else {
...
void save_quantized_model(network net) {
    ...
    for (j = 0; j < net.n; ++j) {
        ...
        if (l->type == CONVOLUTIONAL) {            
            ...
            sprintf(weightfile  , "/app/bin//log_param/CONV%02d_param_weight.hex", j);
            sprintf(biasfile    , "/app/bin//log_param/CONV%02d_param_biases.hex", j);
            sprintf(scalefile   , "/app/bin//log_param/CONV%02d_param_scales.hex", j);
