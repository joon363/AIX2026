...
//{{{   
    ...
    network net = parse_network_cfg("/app/bin/aix2024.cfg", 1, 0);    // parser.c
    int layer_idx = 0;
    ...
    write_from_file_to_fpga(comport_handle, "/app/bin/log_feamap/CONV00_input_32b.hex", base_addr_config[0], 256 * 256);  // Input image
 
    // 2-3. Write weights to DRAM on FPGA
    // Note: Weights are in a 32-bit format now. We can speed up four times with 8-bit weights. 
    write_from_file_to_fpga(comport_handle, "/app/bin/log_param/CONV00_param_weight.hex", base_addr_config[1], 432);     // Only CONV00 now
    //write_from_file_to_fpga(comport_handle, "/app/bin/log_param/CONV10_param_weight.hex", base_address_weight, 1179648);    
 
    // 3. Start the CNN accelerator engine    
    start_engine(comport_handle);   // Do nothing now. 
 
    // 4. Read results from FPGA (DRAM) to Host
    read_from_fpga_to_file(comport_handle, "/app/bin/log_result_fpga/CONV00_input_32b.hex", base_addr_config[0], 256 * 256);
    read_from_fpga_to_file(comport_handle, "/app/bin/log_result_fpga/CONV00_param_weight.hex", base_addr_config[1], 432);
 
    // 5-1. Verify
    verify_result("/app/bin/log_feamap/CONV00_input_32b.hex",   // Input (Src)
        "/app/bin/log_result_fpga/CONV00_input_32b.hex",        // Output (Dst)
        256 * 256);                                                    // Number of compared data
 
    verify_result("/app/bin/log_param/CONV00_param_weight.hex", // Input (Src)
        "/app/bin/log_result_fpga/CONV00_param_weight.hex",     // Output (Dst)
        432);                                                          // Number of compared data
    
    // 5-2. Display
    write_image("/app/bin/test01.jpg", "/app/bin/log_result_fpga/test01-det-quantized.png");
...
//#endif
    ...
    void write_image(char* filename, char* out_filename) {
        // Parameters
        char* cfgfile    = "/app/bin/aix2024.cfg";
        char* weightfile = "/app/bin/aix2024.weights";        
        char* obj_names  = "/app/bin/yolohw.names";
        int quantized    = 1;
        int save_params  = 0;
        float thresh     = 0.24;
        int dont_show    = 1;
        ...
        for (int i = 0; i < net.n; ++i) {
            ...
            if (l.type == YOLO) {
                ...
                snprintf(file_input_femap, sizeof(file_input_femap), "/app/bin/log_result_fpga/CONV%02d_output.hex", i - 1);
